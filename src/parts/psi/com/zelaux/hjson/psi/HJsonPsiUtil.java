...
public class HJsonPsiUtil {
    ...
    public static boolean isPropertyValue(@NotNull PsiElement element) {
        return element instanceof HJsonMemberValue;
    }

    /**
     * Find the furthest sibling element with the same type as given anchor.
     * <p/>
     * Ignore white spaces for any type of element except {@link HJsonElementTypes#LINE_COMMENT}
     ...
     * </ul>
     *
     * @param text presumably result of {@link HJsonStringLiteral#getText()}
     * @return
     */
    @NotNull
     ...
     */
    @NotNull
    public static PsiElement addProperty(@NotNull HJsonObject object, @NotNull HJsonMember property, boolean first) {
        final List<HJsonMember> propertyList = object.getMemberList();
        if (!first) {
    ...
    @NotNull
    public static Set<String> getOtherSiblingPropertyNames(@Nullable HJsonMember property) {
        if (property == null) return Collections.emptySet();
        HJsonObject object = ObjectUtils.tryCast(property.getParent(), HJsonObject.class);
        if (object == null) return Collections.emptySet();
        Set<String> result = new HashSet<>();
        for (HJsonMember jsonProperty : object.getMemberList()) {
