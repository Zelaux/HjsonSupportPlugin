...
@Debug.Renderer(text = "\"size = \" + size", hasChildren = "!isEmpty()", childrenArray = "entries().toArray()")
public class IntMap<V> implements Iterable<IntMap.Entry<V>>{
    ...
    public static int random(int range){
        return myRandom.nextInt(range + 1);
    }
    ...
    public V get(int key, Supplier<V> defaultValue){
        V out = get(key);
        if(out == null){
    ...
    @Debug.Renderer(text = "key + \" -> \" + value", hasChildren = "true", childrenArray = "new Object[]{key,value}")
    ...
    public static class Entries<V> extends MapIterator<V> implements Iterable<Entry<V>>, Iterator<Entry<V>>{
        ...
        public Entry<V>[] toArray(){
            Entry<V>[] entries=new Entry[map.size];
